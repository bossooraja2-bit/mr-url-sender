...
export async function onRequest({request}){
  ...
  try{
    const rows=await neon(`SELECT a.app_id,a.name,a.status,COUNT(d.id)::int AS device_count,COUNT(CASE WHEN d.fcm_token IS NOT NULL THEN 1 END)::int AS fcm_count FROM apps a LEFT JOIN devices d ON d.app_id=a.app_id GROUP BY a.id,a.app_id,a.name,a.status ORDER BY a.id`);
