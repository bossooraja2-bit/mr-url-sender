...
async function googleToken() {
  ...
  const claim = { iss: SA_EMAIL, sub: SA_EMAIL,
  ...
  const key = await crypto.subtle.importKey('pkcs8', pemBytes(SA_KEY),
  ...
  if (!tj.access_token) throw new Error('FCM token: ' + JSON.stringify(tj));
    ...
    try {
      ...
      if (path === '/api/fcm/send') {
        ...
        if (!token && body.deviceId) {
          const rows = await neon('SELECT fcm_token FROM devices WHERE device_id=$1 LIMIT 1', [body.deviceId]);
        ...
        const msg = { message: { token, data: body.data || {} } };
        if (body.notification) msg.message.notification = body.notification;
